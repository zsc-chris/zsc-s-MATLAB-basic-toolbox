...
%	Copyright 2025 Chris H. Zhao
	...
	methods
		function self=function_handle(function_handle_,nargin_,nargout_)
			arguments
				function_handle_(1,1)function_handle=@nop
				nargin_(:,:)double{mustBeTrue("@(nargin_)isequal(nargin_,[])||isscalar(nargin_)&&succeeds(@()mustBeInteger(nargin_))",nargin_,VariableNames="nargin")}=[]
				nargout_(:,:)double{mustBeTrue("@(nargout_)isequal(nargout_,[])||isscalar(nargout_)&&succeeds(@()mustBeInteger(nargout_))",nargout_,VariableNames="nargout")}=[]
			end
			if isequal(nargin_,[])
