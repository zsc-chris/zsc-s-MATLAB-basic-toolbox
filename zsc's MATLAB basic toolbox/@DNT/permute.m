...
%PERMUTE	Permute DNT dimensions
%	ret=PERMUTE(self,dims,finalize=true) returns a permuted DNT where the
%	dimensions are permuted to the new dims. The value will be returned if
%	finalize==true, which allows one to directly feed the output to
...
%
%	Example:
%	>> PERMUTE(DNT([1,2;3,4],["a","b"]),["a","c","b"],false)
%
...
%	Copyright 2025 Chris H. Zhao
	...
	zsc.assert(isequal(sort(DNT.index(self.dimnames,dims)),1:numel(dims)),message("MATLAB:permute:badIndex"));
	out=outclass;
	ret=constructor(zsc.permute(gather(self),DNT.index(self.dimnames,dims)),dims(out{2,2,@sort,DNT.index(self.dimnames,dims)}));
	if finalize
		ret=gather(ret);
	end
end